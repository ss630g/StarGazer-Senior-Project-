...
public class playerMovement : MonoBehaviour
{
    ...
    {
        string currentSeasonString = oll.ReturnCurrentLevel();
        float targetVelocityX = directionalInput.x * moveSpeed;
        if (currentSeasonString == "summer")
        {
            velocity.x = Mathf.SmoothDamp(velocity.x, targetVelocityX - 0.5f, ref velocityXSmoothing, (controller.collisions.below) ? accelerationTimeGrounded : accelerationTimeAirborne);
            velocity.y += (gravity * 1.2f) * Time.deltaTime;
        }
        else
        {
